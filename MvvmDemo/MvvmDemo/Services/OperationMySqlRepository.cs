...
namespace MvvmDemo.Services
{
    ...
    // Цей клас не слідує кращим практикам розробки шару (layer) доступу до даних, щоб не ускладнювати його розуміння.
    ...
    public class OperationMySqlRepository : IOperationRepository
    {
        ...
        public ObservableCollection<OperationTypeModel> GetOperationTypes()
        {
            ...
            {
                // Відкриваємо конекшн. На цьому етапі .NET з'єднається з движком БД і попросить виділити доступний конекшн.
                connection.Open();
                ...
                while (reader.Read())
                {
                    // Читаємо значення поточного рядка і будуємо свій обєкт.
                    var operationType = GetOperationTypeFromReader(reader);

                    operationTypes.Add(operationType);
                }
            }
            // Після завершення блоку using з'єднання з базою даних автоматично закриється.
